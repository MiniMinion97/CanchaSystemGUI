...
export class Explore implements OnInit {
  ...
  constructor() {
    effect(() => {
      const establishments = this.filteredEstablishments();

      if (establishments.length === 0) return;

      this.imageService
        .getImagePreviews(establishments.map(e => e.id))
        .subscribe(images => this.images.set(images));
    });
  }

  protected doesEstablishmentHavePreview(establishmentId: number) {
    if (this.images().length > 0) {
      return this.images().some(image => image.uploadData === establishmentId.toString());
    }
    return false;
  }

  protected getImage(establishmentId: number) {
    return this.imageService.getImageUrl(this.images().find(image => image.uploadData === establishmentId.toString())?.id!);
  }
